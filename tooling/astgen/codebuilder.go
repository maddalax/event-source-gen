...
func (cb *CodeBuilder) BuildToEventMethod(structName, methodName, eventType string, fields []NameType) string {
	return cb.ExecuteTemplate(cb.toEventTemplate, map[string]any{
		"StructName": structName,
		"MethodCall": methodName,
		"EventType":  eventType,
		"Fields":     fields,
...
func (cb *CodeBuilder) BuildEventHandler(events []Event) *CodeBuilder {
	var switchCases strings.Builder
	for _, event := range events {
		_, _ = fmt.Fprintf(&switchCases, `case events.%s: (*handler).On%s(&e)
	`, event.StructName(), event.StructName())
	}

	cb.Append(cb.BuildFunction(Function{
		Name: "Handle",
		Parameters: []NameType{
			{"event", "*sourcing.BaseEvent[any]"},
...
func (cb *CodeBuilder) BuildGetProjections(projectionNames []string) *CodeBuilder {
	var projectionInstances strings.Builder
	...
	for _, name := range m.Keys() {
		projectionInstances.WriteString(fmt.Sprintf("&%s{}, ", name))
	}
	...
	cb.Append(cb.BuildFunction(Function{
		...
		Body: fmt.Sprintf(`
			return []ProjectionHandler{%s}
		`, projectionInstances.String()),
	}))
